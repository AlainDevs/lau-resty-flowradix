...
--
local routing_table = require("configs.routing_table")
...
-- Initialize these at the module level, outside of the function
local cache = ngx.shared.my_cache  -- Assuming you've defined a shared dict named 'my_cache'

local function check_method(route_method, opts_method)
    local cache_key = "method_" .. route_method .. "_" .. (opts_method or "")
    local cached_result = cache:get(cache_key)
    if cached_result == nil then
        cached_result = (route_method == nil or route_method == 0 or
            (opts_method and type(METHODS[opts_method]) == "number" and
            bit.band(route_method, METHODS[opts_method]) ~= 0))
...
local function match_route_opts(route, opts, args)
    ...
    local remote_addr = opts.remote_addr

    if not check_method(route.method, method) then
        return false
    end
    ...
    if matcher_ins then
        local ok, err = matcher_ins:match(remote_addr)
        if err then
    ...
    if route.hosts then
        local matched = false
        local match_against = host
        for i = 1, #route.hosts, 2 do
            if str_find(route.hosts[i+1], ":", 1, true) then
                match_against = vars.http_host or host
            else
                match_against = host
            end
            if match_host(route.hosts[i], route.hosts[i + 1], match_against) then
                if opts_matched_exists then
                    opts.matched._host = route.hosts[i] and ("*" .. route.hosts[i + 1]) or host
                end
                matched = true
                break
            end
    ...
    if route.vars then
        local ok, err = route.vars:eval(vars, opts)
        if not ok then
            if ok == nil then
    ...
    if route.filter_fun then
        ...
        if args then
            ...
            args[0] = nil
            ok = fn(vars or ngx_var, opts, unpack(args, 1, args_len))
        else
            ok = fn(vars or ngx_var, opts)
        end
        if not ok then
            return false
        end
...
-- Create a path-to-routes lookup table
local path_to_routes = {}
for _, value in ipairs(routing_table) do
    path_to_routes[value.path] = value.routes
end

local function set_route_prefixes(matched, path)
    -- Check cache first
    local cached_routes = cache:get(path)
    if cached_routes then
        matched.route_prefixes = cached_routes
        return
    end

    -- Try direct lookup first (fastest)
    local direct_match = path_to_routes[path]
    if direct_match then
        matched.route_prefixes = direct_match
        cache:set(path, direct_match)
        return
    end

    -- If no direct match, use regex matching
    for i = 1, #routing_table do
        local value = routing_table[i]
        if ngx_re_find(path, value.path, "jo") then
            matched.route_prefixes = value.routes
            cache:set(path, value.routes)
            return
        end
    end

    -- If no match found, cache the nil result to avoid future lookups
    cache:set(path, "")  -- Use empty string to represent nil in shared dict
end

local function create_cache_key(route, opts, args)
    -- Create a unique key based on the input parameters
    local key_parts = {
        route.id or tostring(route),  -- Assuming each route has a unique id
        opts.method or "",
        opts.host or "",
        opts.remote_addr or "",
        cjson.encode(opts.vars or {}),
        cjson.encode(args or {})
    }
...
local function match_route(self, path, opts, args)
    -- 1. Use local variables for frequently accessed table fields
    local hash_path = self.hash_path
    local tree = self.tree
    local tree_it = self.tree_it
    local match_data = self.match_data

    -- 2. Move host conversion outside the function if possible
    if opts.host then
    ...
    -- 3. Cache the length of the path
    local path_len = #path

    -- 4. Optimize the first loop
    local routes = hash_path[path]
    if routes then
        for i = 1, #routes do
            local route = routes[i]
            local cache_key = create_cache_key(route, opts, args)
            local cached_result = cache:get(cache_key)
            
            if cached_result == nil then
                cached_result = match_route_opts(route, opts, args)
                cache:set(cache_key, cached_result, 60)  -- Cache for 60 seconds
            end

            if cached_result then
                if opts.matched then
                    opts.matched._path = path
                    set_route_prefixes(opts.matched, path)
                end
                return route
            end
        end
    ...
    -- 6. Use local variables for radix functions
    local radix_tree_search = radix.radix_tree_search
    local radix_tree_up = radix.radix_tree_up

    local it = radix_tree_search(tree, tree_it, path, path_len)
    if not it then
        return nil, "failed to match"
    end

    -- 7. Optimize the second loop
    while true do
        local idx = radix_tree_up(it, path, path_len)
        if idx <= 0 then
            break
        end

        routes = match_data[idx]
        if routes then
            local route = _match_from_routes(routes, path, opts, args)
            if route then
                if opts.matched then
                    -- 8. Move the route_prefixes logic to a separate function
                    set_route_prefixes(opts.matched, path)
                end
                return route
            end
